...
func Test_createConnectionURL(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			"postgres://user:pass@localhost:5432/postgres?connect_timeout=30&sslmode=verify-full&sslrootcert=%2Fpath%2Fto%2Fmy%2Fcert.pem",
		},
		{
	...
	for _, tc := range testCases {
		if out := createConnectionURL(DefaultConnectionInfo(tc.arg).(*connectionInfo)); out != tc.want {
